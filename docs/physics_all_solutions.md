...
Двухатомному идеальному газу в результате изобарического процесса подведено количество теплоты \(Q\). На увеличение внутренней энергии газа расходуется часть теплоты \(\frac{\Delta U}{Q}\), равная:
...
- \(B = \frac{\mu_0 I_2}{2R} - \frac{\mu_0 I_2}{2\pi R}\) **✓**
- \(B = \frac{\mu_0 I_2}{2R} + \frac{\mu_0 I_2}{2\pi R}\)
- \(B = \frac{\mu_0 I_2}{2\pi R} - \frac{\mu_0 I_2}{2R}\)
- \(B = \frac{\mu_0 I_2}{\pi R} - \frac{\mu_0 I_2}{2R}\)
...
Ноль индукции возможен только между проводниками, где поля направлены противоположно. Поскольку \(I_1 > I_2\), точка должна быть ближе к меньшему току (\(I_2\)), то есть на участке d.
...
Проводники сближаются, когда токи в ближайших сторонах контуров направлены согласно (параллельно). Это соответствует рисунку 2.
...
Для отсутствия вращательного момента необходимо симметричное расположение контуров. Для притяжения токи в ближайших сторонах должны быть сонаправлены (параллельны). Это соответствует рисунку 4.
...
По закону Фарадея \(\varepsilon_i = -\frac{d\Phi}{dt}\). ЭДС пропорциональна скорости изменения потока (производной). Если поток меняется по синусоидальному закону (\(\Phi \sim \sin(\omega t)\)), то производная (ЭДС) будет меняться по косинусоидальному закону (\(\varepsilon_i \sim -\cos(\omega t)\)). Начальное отрицательное значение функции соответствует графику под номером 4.
...
При размыкании цепи ток и создаваемое им магнитное поле исчезают. Поток через контур 1 убывает. Чтобы поддержать поток, индукционный ток создаст поле того же направления, что было у тока в цепи 2. По правилу правой руки это соответствует току, направленному против часовой стрелки.
...
![image](../assets/img/Электромагнитная индукция/12.1.png)

![image](../assets/img/Электромагнитная индукция/12.2.png)
...
![image](../assets/img/Электромагнитная индукция/18.1.png)

![image](../assets/img/Электромагнитная индукция/18.2.png)
...
Процесс представлен в координатах \(P\)–\(V\). Линии \(ab\) и \(cd\) — изотермы (\(P \sim 1/V\)). Запишем уравнение Менделеева — Клапейрона: \(P V = \frac{m}{M} R T\). При фиксированной температуре произведение \(PV\) тем больше, чем больше масса газа \(m\). График \(ab\) расположен дальше от начала координат, чем \(cd\), значит \(m_{ab} > m_{cd}\).
